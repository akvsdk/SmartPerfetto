...
| 滑动/动画/Jank | `分析滑动卡顿`、`窗口动画期间哪些内容加载任务占用主线程` | 连续主线程工作、帧内/帧间任务及来源、执行/等待、帧结果 |
...
动画与内容加载并行时，选中整个动画区间并指定目标应用。分析会保留两次 doFrame 之间、首尾边界以及未标注 slice 的主线程执行，再结合帧结果判断影响。任务表包含可回查的 slice、线程和时间信息；缺少调度、请求或源码证据时会说明补证方向，不把所有等待归为 IO，也不凭任务名称证明初始化导致掉帧。
