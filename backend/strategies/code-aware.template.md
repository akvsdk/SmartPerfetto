...
- 保留 `referenceId` / `chunkId`、相对 `filePath` / `lineRange` / `symbol`。`provider_send` 可引用本轮实际读取的必要源码片段来解释结论；不复述 secret/rootPath/absolute path。`metadata_only` / `provider_send_disabled_for_session` 只能定位，不推测源码正文。
